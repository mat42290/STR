...
void Tasks::Init() {
    ...
    if (err = rt_mutex_create(&mutex_comOpened, NULL)) {
        cerr << "Error mutex create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_task_create(&th_watchdogReset, "th_watchdogReset", 0, PRIORITY_TWDRST, 0)) {
        cerr << "Error task create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
...
void Tasks::Run() {
    ...
    if (err = rt_task_start(&th_batteryLevel, (void(*)(void*)) & Tasks::BatteryTask, this)) {
        cerr << "Error task start: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }

    if (err = rt_task_start(&th_watchdogReset, (void(*)(void*)) & Tasks::WatchdogResetTask, this)) {
        cerr << "Error task start: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
...
void Tasks::ServerTask(void *arg) {
    ...
    while(1){
        ...
        cout << "Waiting restart instruction" << endl << flush;
        rt_sem_p(&sem_restart,TM_INFINITE);
        cout << "Rock'n roll restart baby" << endl << flush;
    }
}
...
void Tasks::ReceiveFromMonTask(void *arg) {
    ...
    while (1) {
        ...
        if (msgRcv->CompareID(MESSAGE_MONITOR_LOST)) {
            ...
            cout << "MON ASKS START NO-WD" << endl << flush;
            rt_mutex_acquire(&mutex_withWD,TM_INFINITE);
            withWD = 0;
            rt_mutex_release(&mutex_withWD);
            rt_sem_v(&sem_startRobot);
...
void Tasks::WatchdogResetTask(void * arg){
    int rs;
    int wd;
    Message * ack;
    cout << "Start " << __PRETTY_FUNCTION__ << endl << flush;
...
void Tasks::StartCameraTask(void *arg) {
    ...
    while(1) {
        ...
        if (cameraOpened) {
            ...
            cout << "Camera start success" << endl << flush;
        } else {
